...
def train(args, train_dataset, model, tokenizer, config):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.n_gpu > 1: # we haven't tested multi-gpu setting
...
def evaluate(args, model, processor, tokenizer, config, prefix=""):
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
       ...
       for checkpoint in checkpoints:
           ...
           model.to(args.device)
           result = evaluate(args, model, processor, tokenizer, config, prefix=prefix)
