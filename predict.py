...
from model import BertForSequenceClassificationDistil
...
def predict(args, model, processor, tokenizer, config, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        eval_dataset = load_and_cache_examples(args, eval_task, processor, tokenizer, split="test")
        ...
        if args.defense == "perturb":
            noise_sampler = Normal(0, args.delta)
            ...
            with torch.no_grad():
                ...
                if args.model_type != "distilbert":
                    inputs["token_type_ids"] = (
                        batch[3] if args.model_type in ["bert", "dpbert", "xlnet", "albert", "dstilbert"] else None
                    )  # XLM, DistilBERT, RoBERTa, and XLM-RoBERTa don't use segment_ids
                outputs = model(**inputs)

                if args.defense == "perturb":
                    preds = F.softmax(outputs[1], dim=-1)
                    peturb_probs = torch.clamp(preds + noise_sampler.sample(preds.size()).to(preds.device), min=0, max=1) + 1e-30
                    peturb_probs = peturb_probs / peturb_probs.sum(dim=-1, keepdim=True)
                    preds = peturb_probs.detach().cpu().numpy()
                else:
                    preds = F.softmax(outputs[1]/args.temp, dim=-1).detach().cpu().numpy()

                ids = batch[0].cpu().numpy()

                for id, pred in zip(ids, preds):
                    results.append((id, " ".join([str(p) for p in pred])))
...
def main():
    ...
    parser.add_argument("--temp", default=1, type=float, help="Tempature for softmax")
    parser.add_argument("--delta", default=0.05, type=float, help="delta for Gaussion Dist")
    parser.add_argument("--defense", default="temp", type=str, choices=['temp', 'perturb'], help="Defense strategy")
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
       ...
       for checkpoint in checkpoints:
           ...
           model.to(args.device)
           result = predict(args, model, processor, tokenizer, config, prefix=prefix)
